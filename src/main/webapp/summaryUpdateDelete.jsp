...
<body>
	...
	<div class="container area" style="cursor:pointer;" id="jb-title">
		<table class="table table-striped" style="text-align: center; cellpadding:50px;" >
			<thead>
				...
				<tr>
					<th colspan="5" style=" text-align: center; " data-toggle="tooltip" data-html="true" data-placement="bottom" title="<%= str %>"><%= pl %> 요약본 수정 및 삭제
	...
	<div class="container">
		<div class="row">
		<%
		if(sum.size() == 0 || sum.isEmpty()) { //만약, 미승인 데이터가 없다면!
			PrintWriter script = response.getWriter();
			script.println("<script>");
		...
		%>
			<table id="bbsTable" class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				<tbody>
					...
					<tr>
						...
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<a href="summaryRkUpdate.jsp?sum_id=<%= sum.get(i).getSum_id() %>" data-toggle="tooltip" data-html="true" data-placement="bottom" title="미승인 상태인 경우, 수정 및 삭제가 가능합니다.">
							[<%= pl %>] - summary (<%= dl %>)</a></td>
						<td><%= name %></td>
			...
			<%
				if(pageNumber != 1){
			%>
				<a href="summaryRk.jsp?pageNumber=<%=pageNumber - 1 %>"
			...
			<a href=summaryRk.jsp style="width:50px;" class="btn btn-primary pull-right form-control" data-toggle="tooltip" data-placement="bottom" title="<%= pl %> 요약본 조회" id="summary"> 목록 </a>
		</div>
	</div>
