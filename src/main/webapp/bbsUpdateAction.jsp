	...
	<%
		...
		if(id == null){
			...
			if(bbs.getBbsTitle() == null){
				PrintWriter script = response.getWriter();
				script.println("<script>");
				script.println("alert('주간보고 명세서가 입력되지 않았습니다. (title)')");
					...
					if(bbs.getBbsNTarget() == null){
						...
						NTarget = (String)bbs.getBbsNTarget();
							...
							if(dl != "") {
								...
								int result = bbsDAO.write(id, bbs.getBbsManager(), bbs.getBbsTitle(), name, bbs.getBbsContent(), bbs.getBbsStart(), bbs.getBbsTarget(), bbs.getBbsEnd(), NContent, NStart, NTarget, bbs.getBbsDeadline(), pluser);
								// 데이터베이스 오류인 경우
								if(result == -1){
									PrintWriter script = response.getWriter();
									script.println("<script>");
								...
								// 글쓰기가 정상적으로 실행되면 알림창을 띄우고 게시판 메인으로 이동한다
									...
									PrintWriter script = response.getWriter();
									script.println("<script>");
									script.println("alert('보고가 정상적으로 제출되었습니다.')");
